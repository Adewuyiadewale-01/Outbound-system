...
class RecoveryTests(unittest.TestCase):
    def test_navigation_timeout_after_arrival_uses_readiness(self):
        ...
        with patch('linkedin_helper._wait_for_linkedin_ready', return_value={'ready': True}), patch.object(pe, 'append_history'):
    ...
    def test_unhydrated_activity_does_not_pass_readiness(self):
        ...
        cdp.evaluate.return_value = url
        with patch('linkedin_helper._wait_for_linkedin_ready', return_value={'ready': True}), patch('linkedin_helper._wait_for_activity_feed_state', return_value={'ready': False}), patch.object(pe, 'append_history'):
    ...
    def test_extraction_continues_past_acceptance_floor(self):
        def snapshot(n):
            return {'success': True, 'expected': 100, 'profiles': [{'url': f'https://www.linkedin.com/in/p{i}/'} for i in range(n)], 'scroller': {'left': 0, 'top': 0, 'width': 400, 'height': 400}}
        responses = [{'success': True, 'expected': 100}, snapshot(90), snapshot(97)] + [snapshot(97)] * 5 + [True]
        campaign = {'day': '2026-09-06', 'candidates': [], 'sources': [{'submitted_url': 'https://www.linkedin.com/posts/example'}]}
        with patch.object(pe, '_navigate'), patch.object(pe, '_evaluate_json', side_effect=responses), patch.object(pe, 'save_campaign'), patch.object(pe.time, 'sleep'):
