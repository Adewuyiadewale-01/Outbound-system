    ...
    try:
        ...
        if warm_up.get("error"):
            ...
            for index, prospect in enumerate(selected):
                ...
                if timing == "before_conn":
                    try:
                        profile_data = session.view_profile(profile_url)
                        if profile_data.get("error"):
                            raise RuntimeError(profile_data.get("danger", "profile_view_failed"))
                        activity = session.read_activity(profile_url)
                        if activity.get("error"):
                            raise RuntimeError(activity.get("danger", "activity_read_failed"))
                    except Exception as exc:
                        result["ok"] = False
                        result["status"] = "blocked_browser_action"
                        result["blockers"].append(f"before_conn_profile_activity exception: {exc}")
                        result.setdefault("browser_failures", []).append(
                            {
                                "prospect_id": prospect.get("id"),
                    ...
                    if _activity_classification_uncertain(activity):
                        ...
                        result.setdefault("warnings", []).append(
                            f"Activity classification skipped before send for prospect {prospect.get('id')}: timestamps were not parseable"
                        )
                    else:
                        activity_value = _activity_dropdown_value(activity)
                        activity_fields = build_prospect_activity_fields(prospect, activity_value)
                        ...
                        try:
                            ...
                            result["status"] = "blocked_prospect_sync"
                            result["blockers"].append(
                                f"Prospects activity sync failed before send for prospect {prospect.get('id')}: {exc}"
                            )
                            result["sync_failures"].append(
                                {
                                    "prospect_id": prospect.get("id"),
                ...
                try:
                    send_kwargs = {
                        ...
                        "activity_data": activity if timing == "before_conn" else None,
                    }
                    try:
                        send_result = session.send_connection(
                            profile_url,
                            **send_kwargs,
                        )
                    ...
                    result["skipped"].append(
                        {
                            "prospect_id": prospect.get("id"),
                            "reason": f"send_exception:{failure_key}",
                        }
                    )
                    if failure_key == EMAIL_REQUIRED_TO_CONNECT:
                        _record_requires_email_status(
                        ...
                        _reset_send_failure_guard(result)
                        _sleep_failure_backoff(result, is_last=index >= (len(selected) - 1))
                        continue
                    trip_reason = _register_send_failure(result, prospect.get("id"), failure_key)
                    if trip_reason:
                        result["ok"] = False
                ...
                while (
                    ...
                    try:
                        refreshed_profile = session.view_profile(profile_url)
                        retry_kwargs = {
                            "note": note,
                            "enable_notifications": True,
                            "profile_data": refreshed_profile,
                        }
                        if timing == "before_conn" and activity:
                            retry_kwargs["activity_data"] = activity
                        try:
                            send_result = session.send_connection(profile_url, **retry_kwargs)
                        except TypeError as exc:
                            if "unexpected keyword argument" not in str(exc):
                                raise
                            send_result = session.send_connection(
                        ...
                        send_result = {
                            ...
                            "profile_url": profile_url,
                            "success": False,
                            "error": f"retry_exception:{_normalize_failure_key(exc)}",
                            "prospect_id": prospect.get("id"),
                        }
                        break

                if retry_attempts:
                    send_result["retry_attempts"] = retry_attempts
                result["sent"].append(send_result)
                ...
                if timing != "before_conn":
                    try:
                        session.view_profile(profile_url)
                        activity = session.read_activity(profile_url)
                    except Exception as exc:
                        result["ok"] = False
                        result["status"] = "blocked_browser_action"
                        result["blockers"].append(f"after_conn_profile_activity exception: {exc}")
                        result.setdefault("browser_failures", []).append(
                            {
                                "prospect_id": prospect.get("id"),
                    ...
                    if _activity_classification_uncertain(activity):
                        ...
                        result.setdefault("warnings", []).append(
                            f"Activity classification skipped after send for prospect {prospect.get('id')}: timestamps were not parseable"
                        )
                    else:
                        activity_value = _activity_dropdown_value(activity)
                        activity_fields = build_prospect_activity_fields(prospect, activity_value)
                        ...
                        try:
                            ...
                            result["status"] = "blocked_prospect_sync"
                            result["blockers"].append(
                                f"Prospects activity sync failed after send for prospect {prospect.get('id')}: {exc}"
                            )
                            result["sync_failures"].append(
                                {
                                    "prospect_id": prospect.get("id"),
                ...
                _record_journal_event(
                    ...
                    sheet_payloads={
                        ...
                        "outreach_log": _sheet_target_payload(
                            target="Outreach Log",
                            fields=outreach_log_row,
                            insert_mode="below_header",
                        ),
                        "daily_actions": _sheet_target_payload(
                            target="Daily Actions",
                            row_number=row_number,
                            fields=daily_progress_fields,
                        ),
                    },
                )
                _record_journal_event(
                ...
                # Persist task progress immediately after each confirmed successful send
                # so partial progress is not lost if the session is interrupted later.
                _record_journal_event(
                    result,
                    date_value,
                    "daily_actions_sync",
                ...
                try:
                    if new_progress >= target:
                        result["daily_progress_update"] = complete_daily_row(
                            creds, args.task_manager_url, row_number, progress_value, progress_notes
                        )
                    else:
                        result["daily_progress_update"] = partial_daily_row(
                            creds, args.task_manager_url, row_number, progress_value, progress_notes
                        )
                    _record_journal_event(
                        result,
                    ...
                    result["ok"] = False
                    result["blockers"].append(
                        f"Daily Actions sync failed after successful send for prospect {prospect.get('id')}: {exc}"
                    )
                    result["status"] = "blocked_daily_actions_sync"
                    break

                # Enforce per-lead pacing/diversion between leads.
            ...
            try:
                result["cool_down"] = session.cool_down()
            except Exception as exc:
                result["cool_down"] = {"ok": False, "error": str(exc)}
                result.setdefault("warnings", []).append(f"cool_down exception: {exc}")
