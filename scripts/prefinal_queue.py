...
def rows_fingerprint(rows: List[Dict[str, Any]]) -> str:
    payload = json.dumps(canonical_rows(rows), sort_keys=True, separators=(",", ":"), ensure_ascii=True)
...
def load_batch(path_or_fingerprint: Union[str, Path]) -> Dict[str, Any]:
...
def record_prefinal_publish(
    ...
    *,
    ...
    batch["prefinal_publish"] = {
        ...
        "start_row": start_row,
        "verified": verified,
        "published_at": datetime.now().isoformat(timespec="seconds") if verified and not error else "",
        "error": error,
    }
...
def list_batches() -> List[Dict[str, Any]]:
    ...
    for path in queue_dir().glob("*.json"):
        try:
            ...
            continue
    return sorted(batches, key=lambda batch: (batch.get("created_at", ""), batch.get("fingerprint", "")))


def next_activity_batch() -> Optional[Dict[str, Any]]:
    for batch in list_batches():
        if batch.get("status") in {"queued", "activity_in_progress", "activity_blocked", "final_bridged"}:
            return batch
    return None
