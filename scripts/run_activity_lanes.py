...
def assign_prepared_targets(state: Dict[str, Any], workers: Sequence[Dict[str, Any]]) -> Dict[str, int]:
    ...
    existing = {str(target.get("worker_id") or "") for target in prepared if target.get("worker_id")}
    if existing:
        if not existing.issubset(valid_ids):
            raise ValueError("Prepared session is assigned to workers not enabled in activity_workers.json.")
    else:
...
def workers_for_prepared_session(
    ...
    """
    prepared = state.get("prepared_targets") or []
    existing_ids = {str(target.get("worker_id") or "") for target in prepared if target.get("worker_id")}
    if existing_ids:
...
def ensure_worker_cdp(worker: Dict[str, Any], *, recover: bool = False) -> Dict[str, Any]:
    ...
    if not launch.is_file():
        raise RuntimeError(f"CDP for {worker['id']} is unavailable and launch script is missing: {launch}")
    ...
    for _ in range(20):
        time.sleep(2)
        if cdp_http_healthy(worker["cdp_host"], worker["cdp_port"]):
            return {"ok": True, "action": "browser_restarted" if stopped_pids else "browser_started", "stopped_pids": stopped_pids}
...
def run_lane(
    ...
    *,
    ...
    command = [sys.executable, str(ACTIVITY_SCRIPT), "--date", day, "--activity-only", "--worker-id", worker["id"], "--no-bridges"]
    if limit_per_lane > 0:
    ...
    state = read_json(session_path(day))
    targets = [
        item for item in state.get("prepared_targets", [])
    ...
    timeout_seconds = max(2 * 60 * 60, min(4 * 60 * 60, fixed_seconds + len(targets) * 90 + 30 * 60))
    try:
        result = subprocess.run(command, cwd=str(ROOT), text=True, capture_output=True, env=env, timeout=timeout_seconds)
    except subprocess.TimeoutExpired as exc:
        return {
            "worker_id": worker["id"], "cdp_port": worker["cdp_port"], "command": command,
            "started_at": started_at, "completed_at": datetime.now().isoformat(timespec="seconds"),
            "ok": False, "returncode": None, "timed_out": True, "timeout_seconds": timeout_seconds,
            "summary": {"ok": False, "status": "lane_timeout", "reason": "derived_runtime_budget_exceeded"},
    ...
    return {
        "worker_id": worker["id"], "cdp_port": worker["cdp_port"], "command": command,
        "started_at": started_at, "completed_at": datetime.now().isoformat(timespec="seconds"),
        "ok": result.returncode == 0, "returncode": result.returncode, "timeout_seconds": timeout_seconds,
...
def finalize(day: str) -> Dict[str, Any]:
    command = [sys.executable, str(ACTIVITY_SCRIPT), "--date", day, "--activity-only", "--finalize-only"]
    result = subprocess.run(command, cwd=str(ROOT), text=True, capture_output=True, timeout=15 * 60)
    return {
        "command": command, "ok": result.returncode == 0, "returncode": result.returncode,
        "stdout_tail": result.stdout[-4000:], "stderr_tail": result.stderr[-4000:],
    }


def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run two sequential Activity Check CDP lanes from a prepared session.")
    parser.add_argument("--date", default=date.today().isoformat())
    parser.add_argument("--plan-only", action="store_true", help="Validate the prepared split without launching Chrome or processing targets.")
    parser.add_argument("--normal-gap-minutes", type=int, default=None, help="Override the normal completed-lane handoff gap.")
    parser.add_argument("--failure-gap-minutes", type=int, default=None, help="Override the CDP-unhealthy handoff gap.")
    parser.add_argument("--post-run-404-retry-gap-minutes", type=int, default=None, help="Fresh-session wait before retrying 404-like results.")
    parser.add_argument("--limit-per-lane", type=int, default=0, help="Run at most this many assigned targets per lane (for a bounded test).")
    parser.add_argument("--skip-finalize", action="store_true", help="Do not write Final/Prospects after lanes finish (for E2E tests).")
    parser.add_argument("--test-stop-worker", default="", help="E2E test only: worker id whose Chrome will close after a completed target.")
    parser.add_argument("--test-stop-after-completed", type=int, default=0, help="E2E test only: close the test worker's Chrome after N completed targets.")
    ...
    scheduled_workers = workers_for_prepared_session(state, workers, single_worker_below_total_targets)
    counts = assign_prepared_targets(state, scheduled_workers)
    normal_gap = int(config.get("normal_handoff_gap_minutes", 10) if args.normal_gap_minutes is None else args.normal_gap_minutes)
    failure_gap = int(config.get("failure_handoff_gap_minutes", 5) if args.failure_gap_minutes is None else args.failure_gap_minutes)
    retry_gap = int(config.get("post_run_404_retry_cooldown_minutes", 5) if args.post_run_404_retry_gap_minutes is None else args.post_run_404_retry_gap_minutes)
    ...
    plan = {
        "date": day,
        "workers": [{"id": item["id"], "cdp_port": item["cdp_port"], "targets": counts[item["id"]]} for item in scheduled_workers],
    ...
    while pending:
        ...
        try:
            ...
            if cdp_failure_counts[current["id"]] >= 2:
                print(json.dumps({"ok": False, "status": "paused_for_browser_recovery", **plan, "results": results, "worker_id": current["id"], "error": str(exc), "reason": "same_cdp_failed_twice"}, indent=2))
                return 1
            recovery_needed.add(current["id"])
            alternatives = [worker for worker_id, worker in pending.items() if worker_id != current["id"]]
            if not alternatives:
                print(json.dumps({"ok": False, "status": "paused_for_browser_recovery", **plan, "results": results, "worker_id": current["id"], "error": str(exc), "reason": "no_healthy_alternate_cdp"}, indent=2))
                return 1
            results.append({"worker_id": current["id"], "ok": False, "summary": {"status": "paused_for_browser_recovery"}, "orchestrator_recovery": {"ok": False, "error": str(exc)}})
        ...
        if (
            ...
            and int(result.get("summary", {}).get("completed_this_run") or 0) >= args.test_stop_after_completed
        ):
            test_stop_used = True
        if result["ok"]:
        ...
        if summary_status != "paused_for_browser_recovery":
            print(json.dumps({"ok": False, "status": "lane_failed", **plan, "results": results}, indent=2))
            return 1
        cdp_failure_counts[current["id"]] += 1
        if cdp_failure_counts[current["id"]] >= 2:
            print(json.dumps({"ok": False, "status": "paused_for_browser_recovery", **plan, "results": results, "reason": "same_cdp_failed_twice"}, indent=2))
            return 1
        recovery_needed.add(current["id"])
        alternatives = [worker for worker_id, worker in pending.items() if worker_id != current["id"]]
    ...
    if any(retry_counts.values()):
        ...
        if not all(item["ok"] for item in shutdown.values()):
            print(json.dumps({"ok": False, "status": "paused_for_browser_recovery", **plan, "results": results, "retry_shutdown": shutdown}, indent=2))
            return 1
        state = read_json(state_file)
        ...
        for worker in scheduled_workers:
            ...
            try:
                ...
                print(json.dumps({"ok": False, "status": "paused_for_browser_recovery", **plan, "results": results, "retry_results": retry_results, "worker_id": worker["id"], "error": str(exc), "reason": "fresh_404_retry_cdp_unavailable"}, indent=2))
                return 1
            ...
            if not retry_result["ok"]:
                print(json.dumps({"ok": False, "status": "lane_failed", **plan, "results": results, "retry_results": retry_results}, indent=2))
                return 1
    if args.skip_finalize:
        print(json.dumps({"ok": True, "status": "handoff_test_completed", **plan, "results": results, "retry_results": retry_results, "finalize": {"skipped": True}}, indent=2))
        return 0
    final_result = finalize(day)
    print(json.dumps({"ok": bool(final_result["ok"]), "status": "completed" if final_result["ok"] else "finalize_failed", **plan, "results": results, "retry_results": retry_results, "finalize": final_result}, indent=2))
