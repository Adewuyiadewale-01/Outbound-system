...
def local_false_successes(export_rows: List[Dict[str, str]]) -> Dict[str, Dict[str, Any]]:
    ...
    for journal_path in JOURNALS.glob("*.jsonl"):
        for line in journal_path.read_text(encoding="utf-8", errors="ignore").splitlines():
            ...
            if event.get("event") == "withdrawal_target_result" and event.get("confirmed") is True and prospect_id in targets:
                successes[prospect_id] = {"target": targets[prospect_id], "event": event, "journal": journal_path.name}

    by_slug = {profile_slug(item["target"].get("contact_linkedin", "")): prospect_id for prospect_id, item in successes.items()}
    by_name = {name_key(item["target"].get("contact_name", "")): prospect_id for prospect_id, item in successes.items()}
...
def main() -> int:
    ...
    parser.add_argument("--linkedin-export", required=True, help="Text export copied from LinkedIn Sent Invitations.")
    parser.add_argument("--credentials", default=CREDS_PATH)
    parser.add_argument("--sheet-url", default=OBF_SHEET_URL)
    parser.add_argument("--apply", action="store_true", help="Write the reconciliation. Without this flag, preview only.")
    ...
    live_rows = {str(row[index["Prospect ID"]] if index["Prospect ID"] < len(row) else "").strip(): (row_number, row) for row_number, row in enumerate(values[1:], start=2)}
    ...
    for prospect_id, item in matched.items():
        ...
        progress = str(row[index["Current Progress"]] if index["Current Progress"] < len(row) else "").strip()
        if progress != "Request Withdrawn":
            skipped.append({"prospect_id": prospect_id, "reason": "live_progress_not_request_withdrawn", "live_progress": progress})
            continue
        ...
        for header, value in (("Current Progress", "Conn Request"), ("Outcome", "Pending"), ("Notes", notes)):
            updates.append({"range": a1(row_number, index[header] + 1), "values": [[value]]})
        restored.append({"prospect_id": prospect_id, "row_number": row_number, "company": item["target"].get("company"), "contact_name": item["linkedin_export"]["name"], "prior_reason": item["event"].get("reason", "")})

    report = {"created_at": now, "apply": bool(args.apply), "matched_current_linkedin_pending": len(matched), "restored": restored, "skipped": skipped}
    if args.apply and updates:
        worksheet.batch_update(updates, value_input_option="USER_ENTERED")
    REPORTS.mkdir(parents=True, exist_ok=True)
    report_path = REPORTS / f"{datetime.now(LOCAL_TZ).date().isoformat()}-false-withdrawal-reconciliation.json"
    report_path.write_text(json.dumps(report, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps({"ok": True, "report": str(report_path), "matched": len(matched), "restored": len(restored) if args.apply else 0, "would_restore": len(restored), "skipped": len(skipped)}, indent=2))
    return 0
