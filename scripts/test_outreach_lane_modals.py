...
def main() -> int:
    ...
    for row in rows:
        ...
        if url:
            selected[lane] = {"id": row.get("ID", ""), "company": row.get("Company", ""), "url": url}
    missing = [worker["primary_lane"] for worker in workers if worker["primary_lane"] not in selected]
    if missing:
        raise SystemExit(f"No usable test prospect for lane(s): {', '.join(missing)}")

    prepared = {
        "date": date.today().isoformat(), "test_only": True,
        "purpose": "no_send_connection_modal_e2e", "prepared_at": datetime.now().isoformat(timespec="seconds"),
        "handoff_gap_minutes": 0,
        "lanes": [{"worker_id": by_lane[lane]["id"], "primary_lane": lane, "prospect": item} for lane, item in selected.items()],
    }
    ...
    for worker in workers:
        ...
        child = subprocess.run(
            [sys.executable, str(ROOT / "helpers" / "linkedin_helper.py"), "verify-connection-modal", "--url", prospect["url"]],
        ...
        try:
            ...
            result = {"ok": False, "error": "modal_check_no_result", "stdout": child.stdout[-1000:], "stderr": child.stderr[-1000:]}
        results.append({"worker_id": worker["id"], "primary_lane": worker["primary_lane"], "prospect": prospect, "cdp": recovery, "modal_check": result})
    output = {"ok": all(item["modal_check"].get("ok") for item in results), "test_only": True, "sent": 0, "handoff_gap_minutes": 0, "state_path": str(path), "results": results}
